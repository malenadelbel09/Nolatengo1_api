...
@app.route("/api/v1/users/<user_id_email>", methods=["GET", "PUT", "DELETE"])
def user_manage(user_id_email):
    users_path = app.config['USERS_FILE']
    ...
    if request.method == "GET":
        if os.path.exists(users_path):
            with open(users_path, "r") as file:
                ...
                if any(user['id'] == user_id_email for user in users) | any(user['email'] == user_id_email for user in users):
                    for user in users:
                        if (user['email'] == user_id_email) | (user['id'] == user_id_email):
                            return jsonify({"user": user, "message": "User loaded successfully", "status": "200"})

                return jsonify({"user": user_id_email, "message": "The user not exists", "status": "204"})
                    ...
                    for user in users:
                        if user['id'] == user_id:
                            ...
                            with open(users_path, "w") as f:
                                ...
                                return jsonify({"user": user_updated, "message": "User data updated", "status": "200"})
    ...
    if request.method == "POST":
        ...
        if os.path.exists(cards_path):
            with open(cards_path, "r") as file:
                ...
                if any(card['id'] == new_card['id'] for card in cards): #chequeo que la card que quiero cargar exista en la DB de cards
                            ...
                            if any(user['id'] == user_id for user in users):
                                for user in users:
                                    if user['id'] == user_id:
                                        ...
                                        if not any(card['id'] == new_card['id'] for card in user['cards']): #chequeo que card ya no este cargada en este user
                                            user_aux = user
                                        ...
                                        return jsonify({"card": new_card['id'], "message": "The card already exist", "status": "200"})
                ...
                if any(user['id'] == user_id for user in users):
                    for user in users:
                        if user['id'] == user_id:
                            ...
                            if any(card['id'] == card_id for card in user['cards']): #chequeo que tenga la card q quiero actualizar
                                for card in user['cards']:
                                    if card['id'] == card_id:

                                        user_aux = user
                                        updated_card = card
                                        updated_card['amount'] = updated_amount['amount']

                                        user_aux['cards'].remove(card)
                                        user_aux['cards'].append(updated_card)
                                        ...
                                        with open(users_path, "w") as f:
                                            json.dump(users, f, indent=4)
                                            return jsonify({"card": updated_card, "userId": user_id, "message": "Updated card amount", "status": "200"})

                            return jsonify({"card": card_id, "message": "The card not exist","status": "200"})
...
def trades_manage():
    trades_path = app.config['TRADES_FILE']
    cards_path = app.config['CARDS_FILE']
    users_path = app.config["USERS_FILE"]
    ...
    if request.method == "POST":
        ...
        if os.path.exists(users_path):
            with open(users_path, "r") as file:
                users = json.load(file)
                if any(user['id'] == new_trade['publisher'] for user in users): #check que el id del publicador exista

                    if os.path.exists(cards_path):
                        with open(cards_path, "r") as cards_file:
                            cards = json.load(cards_file)
                            #chequeo que exista el id de las cards(oferta y la que pido a cambio)
                ...
                if any(trade['tradeId'] == trade_id for trade in trades):
                    for trade in trades:
                        if trade['tradeId'] == trade_id:
                            return jsonify({"trade": trade, "message": "Trade loaded successfully", "status": "200"})
                    ...
                    for trade in trades:
                        if trade['tradeId'] == trade_id:
                            ...
                            with open(path, "w") as f:
                                json.dump(trades, f, indent=4)
                                return jsonify({"trade": updated_trade, "message": "Trade data updated", "status": "200"})
    ...
    if request.method == "POST":
        ...
        if not os.path.exists(path):
            ...
            with open(path, "r+") as file:
                ...
                if any(store['address'] == new_store['address'] for store in stores):
                    return jsonify({"store": new_store, "message": "The store already exists", "status": "304"})
...
@app.route("/api/v1/stores/<store_id>", methods=["GET", "PUT", "DELETE"])
                ...
                if any(store['id'] == store_id for store in stores):
                    for store in stores:
                        if store['id'] == store_id:
                            return jsonify({"store": store, "message": "Stpre loaded successfully", "status": "200"})

                return jsonify({"store": store_id, "message": "The storeId not exists", "status": "204"})
            ...
            with open(path, "r+") as file:
                ...
                if any(store['id'] == store_id for store in stores):
                    for store in stores:
                        if store['id'] == store_id:
                            stores.remove(store)
                            store_updated["id"] = store_id
                            stores.append(store_updated)

                            with open(path, "w") as f:
                                json.dump(stores, f, indent=4)
                                return jsonify({"store": store_updated, "message": "store data updated", "status": "200"})
